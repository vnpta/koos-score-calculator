...
// Hàm hiển thị số lượt hoàn thành từ Firestore
async function hienThiSoLuong() {
  try {
    const docRef = doc(db, "1", "dahoanthanh");
    const docSnap = await getDoc(docRef);

    const count = docSnap.exists() ? docSnap.data().soluong || 0 : 0;
    document.getElementById("completionCount").innerText =
      "Số lượt hoàn thành: " + count;
  } catch (error) {
    console.error("Lỗi khi đọc số lượng:", error);
  }
}

// Hàm tăng số lượng hoàn thành
async function tangSoLuongHoanThanh() {
  try {
    const docRef = doc(db, "1", "dahoanthanh");
    const docSnap = await getDoc(docRef);

    const currentCount = docSnap.exists() ? (docSnap.data().soluong || 0) : 0;
    const newCount = currentCount + 1;

    await setDoc(docRef, { soluong: newCount });
    hienThiSoLuong();
  } catch (error) {
...
// Gọi khi trang vừa load để hiển thị số lượng
document.addEventListener("DOMContentLoaded", function () {
  hienThiSoLuong();
});

// Gọi khi người dùng hoàn tất khảo sát
